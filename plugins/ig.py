...
class PinterestMediaDownloader:
    ...
    def get_pin_id(self):
        ...
        self.pin_id = (
            history[-1].headers["location"].split("/")[4]
            if history
            else self.pin_url.split("/")[4]
        )

    def get_pin_data(self):
        self.data = self.session.get(
            self.__class__.info_url.format(self.pin_id)
        ).json()["data"][0]
        ...
        for i, m in enumerate(self.media):
            for s in list(m):
                if m[s]["url"].strip().endswith(".m3u8"):
                    m.pop(s)
            new_m = sorted(
                m.values(), key=lambda s: s["width"] * s["height"], reverse=True
            )
                ...
                for meta_tag in meta_tags:
                    ...
                    try:
                        dump_file = await message.reply_video(
                            f"https://ddinstagram.com{content_value}"
                        )
                    except:
                        downfile = wget.download(
                            f"https://ddinstagram.com{content_value}"
                        )
